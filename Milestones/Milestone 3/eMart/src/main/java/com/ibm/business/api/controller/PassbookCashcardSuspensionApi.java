...
@RestController
@RequestMapping(
        value = ApiConstant.DIGITAL_BASE_PATH + ApiConstant.PASSBOOK_CASHCARD_SUSPENSION_PATH)
public class PassbookCashcardSuspensionApi extends BaseApi {
    /** ロガー　*/
    private static final Logger logger = LogManager.getLogger(PassbookCashcardSuspensionApi.class);

    @Resource(name = "passbookCashcardSuspensionService")
    PassbookCashcardSuspensionService passbookCashcardSuspensionService;
     ...
     * 利用停止・再発行用登録内容照会API
     * <p>
     * GET /api/v1/passbookcashcardsuspension
     * </p>
     ...
     * @return 利用停止・再発行用登録内容照会情報
     */
    @ApiOperation(value = "利用停止・再発行用登録内容照会API(店番、科目、口座番号)",
            notes = "利用停止・再発行用登録内容照会API (店番、科目、口座番号)")
    ...
    @ApiResponses(value = {
            @ApiResponse(code = 200, message = SwaggerConstant.COMMON_HTTP_200_RES_DESC_MESSAGE,
                    response = GetPassbookCashcardSuspensionRes.class),
            ...
            @ApiResponse(code = 500, message = SwaggerConstant.COMMON_HTTP_500_RES_DESC_MESSAGE,
                    response = ErrorResponse.class)})
    public ResponseEntity<GetPassbookCashcardSuspensionRes> getGetPassbookCashcardSuspension(
        ...
        // リモートApi通信
        RemoteApiServerResponse<GetPassbookCashcardSuspensionRes> res =
                passbookCashcardSuspensionService.getGetPassbookCashcardSuspension(headerInfo);
        // レスポンスへ変換
        ResponseEntity<GetPassbookCashcardSuspensionRes> responseEntity =
                new RemoteApiServerResponseConverter<GetPassbookCashcardSuspensionRes>()
     ...
     * 利用停止・再発行用登録内容登録API
     * <p>
     * POST /api/v1/passbookcashcardsuspension
     * </p>
     * @param accessToken BFFアクセストークン
     * @param clientID クライアントID
     * @param transactionID トランザクションID
     ...
     * @param bankCode 銀行コード 
     * @param deviceID デバイスID
     * @param updPassbookCashcardSuspensionReq 利用停止・再発行用登録情報
     * @param httpServletResponse HTTPレスポンス
     * @return 利用停止・再発行用登録内容照会情報
     */
    @ApiOperation(value = "利用停止・再発行用登録内容申込API(店番、科目、口座番号)",
            notes = "利用停止・再発行用登録内容申込API (店番、科目、口座番号)")
    ...
    @ApiResponses(value = {
            ...
            @ApiResponse(code = 500, message = SwaggerConstant.COMMON_HTTP_500_RES_DESC_MESSAGE,
                    response = ErrorResponse.class)})
    public ResponseEntity<UpdPassbookCashcardSuspensionRes> postGetPassbookCashcardSuspension(
            @RequestHeader(name = "Authorization", required = true) String accessToken,
            @RequestHeader(name = "client_id", required = true) String clientID,
            @RequestHeader(name = "transactionID", required = false) String transactionID,
            @RequestHeader(name = "routingInfo", required = true) String routingInfo,
            ...
            @RequestHeader(name = "deviceID", required = true) String deviceID,
            @RequestBody(
                    required = true) UpdPassbookCashcardSuspensionReq updPassbookCashcardSuspensionReq,
            HttpServletResponse httpServletResponse) {

        // 入力チェック
        validateService.validate(updPassbookCashcardSuspensionReq);
        ...
        headerInfo.setToken(accessToken);
        // リモートApi通信
        RemoteApiServerResponse<UpdPassbookCashcardSuspensionRes> res =
                passbookCashcardSuspensionService.postGetPassbookCashcardSuspension(headerInfo,
                        updPassbookCashcardSuspensionReq);
        // レスポンスへ変換
        ResponseEntity<UpdPassbookCashcardSuspensionRes> responseEntity =
                new RemoteApiServerResponseConverter<UpdPassbookCashcardSuspensionRes>()
                        .toResponseEntity(res);
        logger.info("### api result: " + responseEntity);
        return responseEntity;
    }
